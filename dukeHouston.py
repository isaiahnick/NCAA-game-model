...
def load_data(filepath):
    ...
    # Parse all sheets
    Houston_df = xls.parse("Houston Player Stats")
    Duke_df = xls.parse("Duke Player Stats")
    Houston_team_df = xls.parse("Houston Team Stats")
    Duke_team_df = xls.parse("Duke Team Stats")
    
    # Strip column names for consistency
    Houston_df.columns = Houston_df.columns.str.strip()
    Duke_df.columns = Duke_df.columns.str.strip()
    
    # Filter out players marked for exclusion
    if "Disclude?" in Houston_df.columns:
        Houston_df = Houston_df[~(Houston_df["Disclude?"] == "x")]
    
    if "Disclude?" in Duke_df.columns:
        Duke_df = Duke_df[~(Duke_df["Disclude?"] == "x")]
    
    return Houston_df, Duke_df, Houston_team_df, Duke_team_df
...
if __name__ == "__main__":
    filepath = "/Users/isaiahnick/Desktop/Duke Houston Metrics.xlsx"  # Update path as needed
    results = main(filepath, "Houston", "Duke", "team1")